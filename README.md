...
│   └── tts_router.py       #   GET /api/tts (텍스트 → 한국어 TTS)
├── services/               # 비즈니스 로직
...
// static const String baseUrl = 'http://192.168.0.x:8080';
```
...
| `/api/scenario/{id}/reference/{turn}` | GET | 레퍼런스 오디오 파일 반환 |
| `/api/scenario/{id}/process` | POST | **메인 통합 파이프라인** |
| `/api/tts` | GET | 텍스트 → 한국어 TTS (MP3 스트림) |
...
```json
{
  ...
  "total_score": 82.5,
  "prosody_feedback": "억양이 자연스러워요! 잘하고 있어요."
}
```
